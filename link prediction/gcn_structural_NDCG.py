...
from utils import load_data, mask_test_edges, preprocess_graph, get_roc_score, load_data2,  load_data_news, load_data3, load_AN, get_roc_score_GCN
from numpy import *
...
def jaccard_simi(adj):
    ...
    for i in range(adj.shape[0]):
        ...
        two = torch.sum(torch.where(adj[i, :].repeat(adj.shape[0], 1) + adj > 0, ones, zeros), axis=1)
        simi[i,:] = ( one / two )

    simi = 10 * torch.where(torch.isnan(simi), zeros, simi)
    return simi.cuda()
...
def avg_ndcg(x_corresponding, x_similarity, x_sorted_scores, y_ranks, top_k):
    ...
    zeros = torch.zeros_like(idcg)
    ndcg_list = torch.where(torch.isnan(ndcg_list), zeros, ndcg_list)
...
def train_fair(top_k, epoch, model, optimizer, features, adj_norm, jaccard_adj_orig, embedding):
    model.train()
    y_similarity1 = simi(embedding)
    x_similarity = jaccard_adj_orig
...
hidden_emb = None
jaccard_adj_orig = jaccard_simi(adj_orig)
...
for epoch in range(args.epochs):
    ...
    all_ndcg_list_train.append(train_fair(top_k, epoch, model, optimizer, features, adj_norm, jaccard_adj_orig, embedding))
