...
def simi(output):  # new_version
    ...
    a_norm = output / a
    b_norm = output / a
    res = 5 * (torch.mm(a_norm, b_norm.transpose(0, 1)) + 1)  # for scale purpose
...
def lambdas_computation(x_similarity, y_similarity, top_k):
    ...
    # (j, k, i)， i denotes z axis，each slide is from a row in similarity matrix
...
def train_fair(epoch, model_name, adj, jaccard_adj_orig, output):
    model.train()
...
for epoch in range(args.epochs):
    ...
    train_fair(epoch, model_name, adj, jaccard_adj_orig, output)
